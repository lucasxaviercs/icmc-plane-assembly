...
#define SQR 57      // "111001"; -- SQR Rx         -- Rx <- Rx * Rx         Format: < inst(6) | Rx(3) | xxxxxxx >
	...
	switch(state)
	{
		...
		case STATE_DECODE:
			...
			switch(opcode){
                ...
                // ----------------------------------
                case SQR:
                    // Seleciona Rx como entrada A da ULA (via Mux3)
                    selM3 = rx;
                    // Seleciona Rx como entrada B da ULA (via Mux4)
                    selM4 = rx;
                    // Define a operação da ULA como MULT (34)
                    // Como Rx entra em A e em B, a ULA fará Rx * Rx
                    OP = MULT;
                    // Seleciona saída da ULA para o Mux2
                    selM2 = sULA;
                    // Habilita escrita no registrador Rx
                    LoadReg[rx] = 1;
                    // Atualiza flags baseado no resultado da ULA
                    selM6 = sULA;
                    LoadFR = 1;
                    // Retorna para buscar próxima instrução
                    state = STATE_FETCH;
                    break;
                // ---------------------------
